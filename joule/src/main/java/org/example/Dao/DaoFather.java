...
public class DaoFather {
    ...
    public DaoFather(int choseDB, int choseTable) {
        ...
        String content = readFileUtil.MethodReadFile("F:\\ZKZD\\Java项目\\Journal_reptile_20240131\\joule\\", "DBConfig.json");
    ...
    public void MethodCreateSomeObject() {
        try {
            ...
            if (null == connection || connection.isClosed()) {
                connection = DriverManager.getConnection(this.ConnectionURL+this.DBName, this.DBUser, this.DBPass);//创建链接
            }
...
//                    String value = method.invoke(obj).toString();
                    System.out.println(method.invoke(obj).toString());
        ...
        try {
            ...
            for (Method method : methods) {
                ...
                if (method.getName().startsWith("get")) {
                    String columnName = method.getName().replace("get", "");
                    columnList += columnName + ",";
                    String value = method.invoke(obj) == null ? "-" : method.invoke(obj).toString().replace("'", "");
                    //如果为空则替换为-
                    if (method.getReturnType() == String.class) {
                        valueList += "'" + value + "',";
                    } else {
